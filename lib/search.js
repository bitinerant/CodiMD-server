...
function getSearch (s_key, userId, callback) {
  var terms = []
  var re = /((?:\\"|[^ "])+)|"((?:\\"|[^"])+)"/g
  ...
  models.Note.findAll({ // https://sequelize.org/master/manual/querying.html
    limit: 18,
    // WHERE ( NOT "permission" = 'private' OR "ownerId" = {userID} ) AND "content" ...
    where: {
      [Op.or]: [
        {permission: {[Op.ne]: 'private'}},
        {ownerId: userId}
      ],
      content: {[Op.and]: terms},
    },
...
function searchGet (req, res) {
  ...
  getSearch(req.query.q, req.user.id, function (err, search) {
